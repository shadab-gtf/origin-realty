...
export default function OurValues() {
  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (ctx) => {
        const { isDesktop, isTablet, isMobile } = ctx.conditions as any;
        ...
        if (!isDesktop) {
          ...
          tl.fromTo(".lux-text", { scale: 1 }, { scale: 1, ease: "none" }, 0);
        }
      },
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Eyebrow */}
          <p className="lux-text mb-4 text-xs text-[#c8a46a]">
          ...
          <div
            ref={dividerRef}
            className="lux-text mt-12 flex items-center justify-center gap-4 sm:mt-16 sm:gap-6  opacity-0"
          >
            <span className="h-px flex-1 sm:block hidden bg-[#d8c29d]" />
            ...
            <span className="h-px sm:block hidden flex-1 bg-[#d8c29d]" />
          </div>
        </div>
      </div>
    </section>
